...
const Perfil: NextPage = (props: any) => {
    ...
    const [ether, setEther] = useState<number | string>(0)
    return (
        <>
            <DrizzleContext.Consumer>
                {(drizzleContext: any) => {
                    const wei = drizzleContext?.drizzleState?.accountBalances[JSON.parse(props?.sesion).address]
                    const toeth = wei / 1000000000000000000
                    setEther(toeth.toFixed(4))
                    return (
                        <>
                            <Head>
                                <title>Perfil</title>
                                    ...
                                    <div className="relative w-full h-full px-4 sm:px-6 lg:px-4 flex items-center">
                                        <div>
                                            ...
                                            <h3 className="text-white text-lg mt-2 ">
                                                <span className='font-semibold text-blue-200'>{JSON.parse(props?.sesion)?.name}</span>
                                            </h3>
                                        </div>
                                    </div>
                                </div>
...
export async function getServerSideProps(ctx: any) {
    ...
    var urlipfs = process.env.IPFS_URL
